...
import net.leeloo.leeloomod.block.custom.CornCropBlock;
import net.leeloo.leeloomod.block.custom.StrawberryCropBlock;
...
public class ModBlockStateProvider extends BlockStateProvider {
    public ModBlockStateProvider(PackOutput output, ExistingFileHelper exFileHelper) {
        super(output, LeelooMod.MOD_ID, exFileHelper);
    }

    @Override
    protected void registerStatesAndModels() {
        blockWithItem(ModBlocks.LAVA_SPONGE_BLOCK);
        blockWithItem(ModBlocks.LAVA_SPONGE_BLOCK_FULL);
        blockWithItem(ModBlocks.EMBEDDED_AMBER);
        blockWithItem(ModBlocks.EMBEDDED_MAUVINITE);
        blockWithItem(ModBlocks.ENCAPSULATED_MAUVINITE);
        //blockWithItem(ModBlocks.MAUVINITE_ORE);
        blockWithItem(ModBlocks.STRANGE_STONE);
        blockWithItem(ModBlocks.AMBER_BLOCK);
        blockWithItem(ModBlocks.ETHEREAL_ORE);

        simpleBlockWithItem(ModBlocks.MAUVINITE_REFINER.get(),
                new ModelFile.UncheckedModelFile(modLoc("block/mauvinite_refiner")));
        ...
        axisBlock(((RotatedPillarBlock) ModBlocks.STRIPPED_PINE_LOG.get()), blockTexture(ModBlocks.STRIPPED_PINE_LOG.get()),
                new ResourceLocation(LeelooMod.MOD_ID, "block/stripped_pine_log_top"));
    ...
    private void blockItem(RegistryObject<Block> blockRegistryObject) {
        simpleBlockItem(blockRegistryObject.get(), new ModelFile.UncheckedModelFile(LeelooMod.MOD_ID +
    ...
    private ConfiguredModel[] strawberryStates(BlockState state, CropBlock block, String modelName, String textureName) {
        ...
        models[0] = new ConfiguredModel(models().crop(modelName + state.getValue(((StrawberryCropBlock) block).getAgeProperty()),
                new ResourceLocation(LeelooMod.MOD_ID, "block/" + textureName + state.getValue(((StrawberryCropBlock) block).getAgeProperty()))).renderType("cutout"));
    ...
    private ConfiguredModel[] cornStates(BlockState state, CropBlock block, String modelName, String textureName) {
        ...
        models[0] = new ConfiguredModel(models().crop(modelName + state.getValue(((CornCropBlock) block).getAgeProperty()),
                new ResourceLocation(LeelooMod.MOD_ID, "block/" + textureName + state.getValue(((CornCropBlock) block).getAgeProperty()))).renderType("cutout"));
