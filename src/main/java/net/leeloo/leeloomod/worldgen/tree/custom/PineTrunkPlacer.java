package net.leeloo.leeloomod.worldgen.tree.custom;
...
public class PineTrunkPlacer extends TrunkPlacer {
    ...
    public List<FoliagePlacer.FoliageAttachment> placeTrunk(LevelSimulatedReader pLevel, BiConsumer<BlockPos, BlockState> pBlockSetter,
        ...
        if(pRandom.nextInt(100) == 99){
            for(int i = 195 + pRandom.nextInt(50); i < height + pRandom.nextInt(50); i++) {
                if (pRandom.nextInt(50) == 49) {
                    pBlockSetter.accept(pPos.atY(i).relative(Direction.UP), ((BlockState)
                            Function.identity().apply(pConfig.trunkProvider.getState(pRandom, pPos))));
                }
                for (int x = 1; x < 5; x++) {
                    if (pRandom.nextInt(50) == 49) {
                        pBlockSetter.accept(pPos.atY(i).relative(Direction.NORTH, x), ((BlockState)
                                Function.identity().apply(pConfig.trunkProvider.getState(pRandom, pPos))));
                    }
                    if (pRandom.nextInt(50) == 49) {
                        pBlockSetter.accept(pPos.atY(i).relative(Direction.SOUTH, x), ((BlockState)
                                Function.identity().apply(pConfig.trunkProvider.getState(pRandom, pPos))));
                    }
                    if (pRandom.nextInt(50) == 49) {
                        pBlockSetter.accept(pPos.atY(i).relative(Direction.EAST, x), ((BlockState)
                                Function.identity().apply(pConfig.trunkProvider.getState(pRandom, pPos))));
                    }
                    if (pRandom.nextInt(50) == 49) {
                        pBlockSetter.accept(pPos.atY(i).relative(Direction.WEST, x), ((BlockState)
                                Function.identity().apply(pConfig.trunkProvider.getState(pRandom, pPos))));
                    }
                }
            }
